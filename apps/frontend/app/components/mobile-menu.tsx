...
//
// The overlay + panel are portaled to document.body. The nav uses backdrop-blur,
// which creates a containing block for `position: fixed` — without a portal the
        ...
        {open ? (
          <div className="fixed inset-0 z-50">
            ...
            <motion.div
              ...
              aria-label="Menu"
              tabIndex={-1}
              className="absolute inset-y-0 right-0 flex h-dvh w-[min(20rem,85vw)] flex-col border-l border-(--color-border) bg-(--color-background) pt-[max(0.75rem,env(safe-area-inset-top))] pr-[max(1rem,env(safe-area-inset-right))] pb-[max(1.5rem,env(safe-area-inset-bottom))] pl-4 shadow-(--shadow-md) outline-none overscroll-contain"
            ...
            >
              ...
              {!isPending && user ? (
                <div className="mb-2 flex shrink-0 items-center gap-2.5 rounded-lg bg-(--color-surface) px-3 py-2.5">
                  <span
              ...
              <div className="shrink-0 border-t border-(--color-border) pt-4">
                {isPending ? (
